...
export class Room {
    ...
    messageHandlers: {
        [messageType: string]: (player: Player, data: any) => void;
    };
    ...
    private makeTopics() {
        ...
        for (let topic in globals.topics) {
            this.topics[topic] =
                globals.topics[topic as keyof typeof globals.topics] +
                "-" +
                this.uuid;
        }
     ...
     */
    public moveDistanceAllowed(
        now: number,
        player: Player,
        otherPosition: globals.PositionData,
        ...
        const distanceTraveled =
            distances[0] + distances[1] - (includeRadius ? 20 : 0);

        let shouldTravelDistance = this.simulator.getMaxDistanceChange(
            now - player.pacman.lastClientTimestamp,
            player.pacman.movementSpeed,
            tolerance
        );

        if (distanceTraveled > shouldTravelDistance) {
            if (this.enable_lagback)
                console.log(
                    `Player ${player.session} (${player.pacman.color}) moved too quickly!`,
                    shouldTravelDistance,
                    distances
                );
            return false;
        }
     ...
     */
    public verifyNewPosition(
        player: Player,
        newPosition: globals.PositionData
    ) {
    ...
    public handlePositionUpdate(
        player: Player,
        data: { data: globals.PositionData }
    ) {
    ...
    public handleWarpTunnelUse(player: Player, data: { data: any }) {
        ...
        for (let node of this.gameBoard.pathIntersections) {
            if (
                node.type == PATH_INTERSECTION_TYPES.NORMAL ||
        ...
        let newPacmanPosition: globals.PositionData = {
            ...player.pacman.lastLocation,
        };
    ...
    public handlePlayerCollision(
        ...
        const now = performance.now();
        if (
            now - player.lastCollision < 500 &&
            now - otherPlayer.lastCollision < 500
        ) {
        ...
        let estimatedOtherPlayerPosition =
            otherPlayer.pacman.getEstimatedPosition(
                performance.now() - otherPlayer.pacman.lastPosPacketTime
            );
        ...
        let dx = Math.abs(
            player.pacman.lastLocation.x - estimatedOtherPlayerPosition.x
        );
        let dy = Math.abs(
            player.pacman.lastLocation.y - estimatedOtherPlayerPosition.y
        );
        ...
        if (player.pacman.isPoweredUp == otherPlayer.pacman.isPoweredUp) {
            this.handlePlayerBump(
                player,
                dx,
                dy,
                now,
                estimatedOtherPlayerPosition,
                otherPlayer
            );
            return;
        }
    ...
    public handlePlayerBump(
        player: Player,
        dx: number,
        dy: number,
        now: number,
        estimatedOtherPlayerPosition: { x: number; y: number },
        otherPlayer: Player
    ) {
    ...
    public handlePelletEat(player: Player, data: any) {
        ...
        if (
            ...
            player.ws.send(
                utils.makeMessage("pellet-reject", {
                    pelletID: data.data.pelletID,
                })
            );
            return;
        }
        ...
        for (
            pellet_index = 0;
            pellet_index < this.gameBoard.pellets.length;
            pellet_index++
        ) {
        ...
        if (pellet == undefined) {
            player.log(
                "Attempted to eat non-existent pellet: " + data.data.pelletID
            );
            player.ws.send(
                utils.makeMessage("pellet-reject", {
                    pelletID: data.data.pelletID,
        ...
        if (distance_from_pellet[0] > 40 || distance_from_pellet[1] > 40) {
            player.log(
                "Attempted to eat pellet too far away from new pos: ",
                distance_from_pellet,
                pellet.id
            );
            player.ws.send(
                utils.makeMessage("pellet-reject", {
        ...
        if (this.gameBoard.pellets.length == 1) {
            ...
            this.server.publish(
                this.topics.event,
                utils.makeMessage("board-state", this.makeBoardState())
            );
            return;
        }
        ...
        this.server.publish(
            this.topics.event,
            utils.makeMessage("eat-pellet", {
                pelletID: pellet.id,
                scores: this.makeScoresList(),
            })
        );
    ...
    public handlePowerPelletEat(player: Player) {
        ...
        player.pacman.powerupTime =
            performance.now() + globals.animation_timings.power_up;
        ...
        player.timers.set(
            ...
            setTimeout(() => {
                ...
                const estimated_pos = player.pacman.getEstimatedPosition(
                    performance.now() - player.pacman.lastPosPacketTime
                );
    ...
    public handleGhostEat(player: Player, data: { data: any }) {
        if (!player.pacman.isPoweredUp) {
            player.ws.send(
                utils.makeMessage("reject-ghost-eat", {
                    id: data.data.ghost_id,
                })
            );
            return;
        }
        ...
        this.server.publish(
            this.topics.event,
            utils.makeMessage("update-scores", {
                scores: this.makeScoresList(),
            })
        );
    ...
    public handlePlayerDead(
        player: Player,
        data: { data: { position: globals.PositionData } }
    ) {
        ...
        player.ws.publish(
            this.topics.event,
            utils.makeMessage("kill-pacman", { id: player.session })
        );
    ...
    public handlePlayerJoin(ws: ServerWebSocket<globals.SocketData>) {
        ...
        const newPlayer = new Player(
            ws.data.session,
            ws,
            utils.getRandomListItem(this.availableColors),
            this
        );
        ...
        newPlayer.ws.publish(
            this.topics.event,
            utils.makeMessage("player-join", {
                session: ws.data.session,
                color: newPlayer.pacman.color,
            })
        );
            ...
            newPlayer.ws.send(
                utils.makeMessage("player-join", {
                    ...
                    "last-location": {
                        ...
                        data: {
                            ...player.pacman.lastLocation,
                            isAlive: player.pacman.isAlive,
                            poweredUp: player.pacman.isPoweredUp,
                        },
                    },
                })
            );
        ...
        console.log(
            `Player ${newPlayer.session} (${newPlayer.pacman.color}) joined`
        );
    ...
    public handlePlayerLeave(session: string) {
        ...
        console.log(
            `Player ${player.session} (${player.pacman.color}) disconnected`
        );

        player.ws.publish(
            this.topics.event,
            utils.makeMessage("player-leave", { session: session })
        );
