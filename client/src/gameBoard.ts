...
class GameBoard {
    ...
    constructor(
        ...
        this.drawLines = this.findDrawLines(this.pixelBlockPositions);

        // do the same as above for pellets
        this.pellets = rawPellets;
        this.pixelPellets = rawPellets.map(
            (p) => new Pellet(p.x * tileSize, p.y * tileSize, p.id, p.type)
        );
    ...
    public findDrawLines(
        ...
        for (let i = 0; i < blockPositions.length; i++) {
            ...
            for (let j = 0; j < blockPositions.length; j++) {
                ...
                const [otherX, otherY, otherWidth, otherHeight] =
                    blockPositions[j];

                // Check for adjacent lines and split them
                remainingTop = this.splitLine(remainingTop, [
                    otherX,
                    otherY + otherHeight,
                    otherX + otherWidth,
                    otherY + otherHeight,
                ]);
                remainingBottom = this.splitLine(remainingBottom, [
                    otherX,
                    otherY,
                    otherX + otherWidth,
                    otherY,
                ]);
                remainingLeft = this.splitLine(remainingLeft, [
                    otherX + otherWidth,
                    otherY,
                    otherX + otherWidth,
                    otherY + otherHeight,
                ]);
                remainingRight = this.splitLine(remainingRight, [
                    otherX,
                    otherY,
                    otherX,
                    otherY + otherHeight,
     ...
     */
    public getTileCoordinatesFromPosition(
        position: Array<number>
    ): [number, number] {
        return [
            Math.ceil(position[0] / gameManager.tileSize - 1),
            Math.ceil(position[1] / gameManager.tileSize - 1),
        ];
    }
    ...
    public getWallAtPosition(position: [number, number]): [number, number, number, number] | null {
        ...
        for (let i = 0; i < this.pixelBlockPositions.length; i++) {
            ...
            if (!pointIntersectsRect(tilePosition, this.blockPositions[i]))
                continue;

            // return this wall
            return this.pixelBlockPositions[i] as [
                number,
                number,
                number,
                number
     ...
     */
    public getNextIntersectionNode(
        ...
        for (let i = 0; i < this.pathIntersections.length; i++) {
            ...
            // TODO: preprocess
            const node = [
                this.pathIntersections[i].x,
                this.pathIntersections[i].y,
            ] as [number, number];

            // if the node is outside our tolerance, then ignore it
            if (
                Math.abs(
                    position[otherCheckDirection] - node[otherCheckDirection]
                ) > tolerance
            )
                continue;

            const distance =
                (position[checkDirection] - node[checkDirection]) *
                checkOrientation;
            ...
            if (
                minDistanceData.node == null ||
                distance < minDistanceData.distance
            ) {
